...
cargo build
cargo test                             # ~164 unit tests, all inline (#[cfg(test)])
...
- Cache is stored at `<repo>/.cache/taoki/` (gitignored): `radar.json`, `xray.json`, `deps.json`. All share a single version (`CACHE_VERSION` in `src/cache.rs`) — bump it when any format changes. Radar uses per-file blake3 hashes (full replacement each call). Xray uses per-file blake3 hashes (upsert per call, pruned during radar). Deps uses two-layer invalidation: per-file content hashes (skip tree-sitter re-parsing) + a fingerprint over file list and workspace config (trigger re-resolution from cached raw imports).
