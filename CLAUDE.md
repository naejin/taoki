...
- **`index/body.rs`** — Body analysis for function/method declarations. `analyze_body()` walks function bodies using tree-sitter (skipping nested functions, closures, and class definitions) and extracts three kinds of insights: call graph (`extract_calls`), match/switch arms (`extract_match_arms`), and error return sites (`extract_error_returns`). Call extraction uses AST-based priority ordering: free functions and scoped calls (domain orchestration) appear before method calls (plumbing), determined by the call-site's AST node kind (e.g., `identifier`/`scoped_identifier` vs `field_expression` in Rust). Method calls fill the remaining MAX_CALLS budget after primary calls. Results are attached to `SkeletonEntry` via a `BodyInsights` struct and rendered by `format_lines()`. Supports all 6 languages.
...
- Cache is stored at `<repo>/.cache/taoki/` (gitignored): `code-map.json` (v5, with tags, skeletons, docstrings, and priority-ordered calls), `deps.json` (dependency graph).
